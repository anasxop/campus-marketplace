      ...
      <div className="home-section" style={{ maxWidth: 1200, margin: "0 auto", padding: "56px 32px 0", boxSizing: "border-box" }}>
        ...
        <div className="home-category-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
          {CATEGORIES.map((cat, i) => {
            ...
            return (
              <div key={cat.label} style={{ background: cat.gradient, borderRadius: 24, padding: "28px 24px 24px", cursor: "pointer", position: "relative", overflow: "hidden", aspectRatio: "1.4 / 1", minHeight: 150, display: "flex", flexDirection: "column", justifyContent: "space-between", transition: "transform 260ms cubic-bezier(0.34,1.56,0.64,1), box-shadow 260ms ease", transform: isHov ? "translateY(-10px) scale(1.02)" : "translateY(0) scale(1)", boxShadow: isHov ? `0 32px 80px ${cat.glowColor}, 0 8px 32px rgba(14,0,40,0.18), inset 0 1px 0 rgba(255,255,255,0.25)` : `0 8px 32px ${cat.glowColor.replace("0.45","0.25")}, inset 0 1px 0 rgba(255,255,255,0.20)`, animation: `fadeUp 0.5s cubic-bezier(0.16,1,0.3,1) ${i * 80}ms both` }} onClick={() => browseByCategory(cat.cat)} onMouseEnter={() => setHoveredCat(i)} onMouseLeave={() => setHoveredCat(null)}>
                {/* Apple glass shine */}
